    ...
    for meta_key, opciones in claves_map.items():
        ...
        for key in opciones:
            if key in d:
                ...
                break
        if not found:
            raise KeyError(f"No se encontró datos para {meta_key} (buscado: {opciones})")
...
def plot_figure3_corrected():
    ...
    for mode in modes:
        ...
        try:
            ...
            if np.sum(L) == 0:
                print(f" [WARN] El modo '{mode}' tiene VOLUMEN 0 en el paso final. Gráficas serán vacías.")
    ...
    if not hay_datos:
        print(" [!] No se pudieron extraer datos válidos de ningún modo. Abortando gráficas.")
        return
    ...
    for mode in modes:
        if mode in data_store:
            ...
            if np.sum(R) == 0:
                print(f" [INFO] DebtRank es 0 para todos en '{mode}' (Sistema muy seguro o sin deuda).")
            
            plt.plot(x_axis, R[rank_indices], marker='o', label=labels[mode], color=colors[mode])
