...
class CRISIS_Model:
    ...
    def record_history(self):
        ...
        self.step_buffer["hh_employer_idx"].append(
            self.hh_employer_idx.astype(np.int16)
        )
    ...
    def step_firms_planning(self):
        ...
        self.firms_state[:, self.IDX_FIRM_PRODUCTION] = (
    ...
    def step_banking_market(self):
        ...
        chosen_bank_ids = pool_indices[
            np.arange(Params.F), best_choice_idx
        ]  # Shape (F,)
        ...
        if len(deficit_ids) > 0 and len(surplus_ids) > 0:
            ...
            if len(pairs_d) > 0:
                ...
                loans_to_banks = np.sum(
                    self.L_bb, axis=1
                )  # (B,) L_bb rows=borrower? No, rows=borrower means i owes j.
    ...
    def step_accounting(self):
        ...
        total_payment_by_bank = np.sum(
            repayment_interbank, axis=1
        )  # Row Sum = What I owe (Borrower)
        total_receipt_by_bank_ib = np.sum(
            repayment_interbank, axis=0
        )  # Col Sum = What I receive (Lender)
        ...
        # Add to HH
        self.households_state[:, self.IDX_HH_DEPOSITS] += per_capita_div[
            self.hh_employer_idx
        ]
        ...
        dividends_b = np.minimum(
            dividends_b, self.banks_state[:, self.IDX_BANK_LIQUIDITY]
        )
        ...
        self.households_state[:, self.IDX_HH_DEPOSITS] += per_capita_div_b[
            self.hh_bank_idx
        ]
        ...
        if len(dead_firms_indices) > 0:
            ...
            bad_loans_per_bank = np.sum(
                self.L_fb[dead_firms_indices, :], axis=0
            )  # (B,)
            ...
            init_liq = self.rng.uniform(
                Params.INIT_FIRM_ASSETS[0],
                Params.INIT_FIRM_ASSETS[1],
                size=len(dead_firms_indices),
            )
        ...
        if len(final_dead_banks) > 0:
            init_assets = self.rng.uniform(
                Params.INIT_BANK_ASSETS[0],
                Params.INIT_BANK_ASSETS[1],
                size=len(final_dead_banks),
            )
            # Reset Equity
            # Assets = Equity + Liabilities. Assume 0 Liabilities initially.
            self.banks_state[final_dead_banks, self.IDX_BANK_EQUITY] = (
                init_assets * Params.INIT_CAPITAL_RATIO
            )  # using Initial Capital Ratio
            self.banks_state[final_dead_banks, self.IDX_BANK_LIQUIDITY] = (
                init_assets  # Cash
            )
