...
def generar_gif_interbancario(data, output_file, duration):
    ...
    for t in steps:
        ...
        ax.set_title(f"Red Interbancaria (t={t})\nVerde: Solvente | Rojo: Default", fontsize=14)
        plt.axis('off')
        
        fname = f"{temp_dir}/frame_{int(t):03d}.png"
        plt.savefig(fname, dpi=100, bbox_inches='tight')
        plt.close()
        frames.append(fname)
        
        if t % 20 == 0: print(f"   Renderizando t={t}")
    ...
    for fname in frames: os.remove(fname)
    os.rmdir(temp_dir)
...
def generar_gif_macro(data, output_file, duration):
    ...
    for t in steps:
        ...
        colors_B = ['#FF4444' if (i in banks_t.index and banks_t.loc[i, 'eq'] < 0) else '#4444FF' for i in range(p.B)]
        colors_F = ['#FF4444' if (i in firms_t.index and firms_t.loc[i, 'eq'] < 0) else '#44FF44' for i in range(p.F)]
        
        # Nodos
        nx.draw_networkx_nodes(G_layout, pos, nodelist=nodes_B, node_color=colors_B, 
                               node_size=400, edgecolors='black', ax=ax)
        nx.draw_networkx_nodes(G_layout, pos, nodelist=nodes_F, node_color=colors_F, 
                               node_size=100, alpha=0.8, ax=ax)
        nx.draw_networkx_labels(G_layout, pos, labels={n: str(n) for n in nodes_B}, 
        ...
        if eb:
            nx.draw_networkx_edges(G_layout, pos, edgelist=eb, width=1.5, edge_color='black', alpha=0.6, ax=ax)
        ...
        if ef:
            nx.draw_networkx_edges(G_layout, pos, edgelist=ef, width=0.3, edge_color='gray', alpha=0.3, ax=ax)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generador de visualizaciones de red (GIF)")
    parser.add_argument("--mode", type=str, choices=["interbank", "macro", "both"], default="both", 
                        help="Tipo de visualización a generar")
    parser.add_argument("--dir", type=str, default=DEFAULT_DATA_DIR, help="Directorio de datos de la simulación")
    parser.add_argument("--speed", type=float, default=DEFAULT_DURATION, help="Duración por frame en segundos (mayor = más lento)")
    ...
    if data:
        ...
        if args.mode in ["interbank", "both"]:
            generar_gif_interbancario(data, "output_plots/red_interbancaria.gif", args.speed)
