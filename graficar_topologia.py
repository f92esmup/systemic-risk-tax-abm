...
def plot_network_topology(mode="none", run_id=0, step_t=-1):
    ...
    if edges_list:
        ...
        )  # Curvar ligeramente para ver bidireccionales
    ...
    sm = plt.cm.ScalarMappable(
        cmap=plt.cm.RdYlGn_r,
        norm=plt.Normalize(vmin=0, vmax=np.max(R) if np.max(R) > 0 else 1),
    )
    sm.set_array([])
    cbar = plt.colorbar(sm, ax=plt.gca(), label="Systemic Risk (DebtRank)", shrink=0.8)

    plt.title(
        f"Interbank Network Topology\nMode: {mode.upper()} | Step: {step_t}",
        fontsize=16,
    )
    plt.axis("off")
