...
def ejecutar_paso2(modelo):
    ...
    if np.any(valid_mask) and modelo.tax_mode == "srt" and modelo.tax_param > 0:
        ...
        est_taxes = fn.calcular_impuesto_srt(
            curr_L, prop_indices, prop_amounts, equity, v, p_default, modelo.tax_param
        )
    ...
    est_ib_rate = fn.calcular_tasa_interbancaria(
        Parametros.R_BAR, avg_psi, est_fragility
    )
    ...
    total_taxes = 0.0
    bank_actual_refinancing_costs = np.zeros(
        Parametros.B
    )  # Total cost (Interest + Tax)
    ...
    for d in deficit_ids:
        ...
        lenders_psi = modelo.estado_bancos[
            potential_lenders, Parametros.IDX_BANK_INTERBANK_COST_PSI
        ]
        ...
        base_rates = fn.calcular_tasa_interbancaria(
            Parametros.R_BAR, lenders_psi, d_fragility
        )
        ...
        if modelo.tax_mode == "srt" and modelo.tax_param > 0:
            p_default = (
                Parametros.DEFAULT_PROB_SCALING
                * fn.calcular_fragilidad_financiera(curr_lev, Parametros.K_mu)
            )
            # Batch: d -> all lenders
            prop_indices = np.column_stack(
                (np.full(len(potential_lenders), d), potential_lenders)
            )
        ...
        unit_costs = np.divide(
            total_costs,
            amounts_to_test,
            out=np.zeros_like(total_costs),
            where=amounts_to_test > 1e-9,
        )
        ...
        for idx in sorted_lender_idxs:
            ...
            if modelo.tax_mode == "srt" and modelo.tax_param > 0:
                ...
                curr_lev_dyn = (curr_deposits + np.sum(curr_L_dyn, axis=1)) / (
                    curr_E_dyn + 1e-9
                )
                p_def_dyn = (
                    Parametros.DEFAULT_PROB_SCALING
                    * fn.calcular_fragilidad_financiera(curr_lev_dyn, Parametros.K_mu)
                )
            ...
            rate_final = base_rates[
                idx
            ]  # Use the rate quoted (assuming it doesn't change wildly)
            ...
            if (prev_amt + actual_amount) > 1e-9:
                new_avg = (prev_amt * prev_rate + actual_amount * rate_final) / (
                    prev_amt + actual_amount
                )
    ...
    if np.any(mask_pos):
        numerator = (old_debt[mask_pos] * old_rates[mask_pos]) + (
            approved_amounts[mask_pos] * final_rates_chosen[mask_pos]
        )
