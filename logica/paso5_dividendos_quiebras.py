...
def ejecutar_paso5(modelo):
    ...
    if len(dead_firms_indices) > 0:
        ...
        modelo.estado_firmas[dead_firms_indices, Parametros.IDX_FIRM_EQUITY] = (
            init_liq
        )
        modelo.estado_firmas[dead_firms_indices, Parametros.IDX_FIRM_PRICE] = (
            init_price
        )
    ...
    while True:
        ...
        current_defaults = np.where((equity < 0) & 
                                    (~np.isin(np.arange(Parametros.B), list(processed_defaults))))[0]
    ...
    if len(processed_defaults) > 0:
        ...
        modelo.estado_bancos[dead_banks, Parametros.IDX_BANK_TOTAL_ASSETS] = init_bank_assets
        modelo.estado_bancos[dead_banks, Parametros.IDX_BANK_EQUITY] = (
            init_bank_assets * Parametros.INIT_CAPITAL_RATIO
        )
        modelo.estado_bancos[dead_banks, Parametros.IDX_BANK_LIQUIDITY] = init_bank_assets
        modelo.estado_bancos[dead_banks, Parametros.IDX_BANK_DEPOSITS] = (
            init_bank_assets - modelo.estado_bancos[dead_banks, Parametros.IDX_BANK_EQUITY]
        )
        
        # Reset behaviors
        modelo.estado_bancos[dead_banks, Parametros.IDX_BANK_OPERATING_COST_CHI] = (
            modelo.rng.uniform(Parametros.CHI_RANGE[0], Parametros.CHI_RANGE[1], size=n_dead)
        )
        modelo.estado_bancos[dead_banks, Parametros.IDX_BANK_INTERBANK_COST_PSI] = (
            modelo.rng.uniform(Parametros.PSI_RANGE[0], Parametros.PSI_RANGE[1], size=n_dead)
        )
