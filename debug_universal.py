...
def diagnosticar():
    ...
    try:
        ...
        if "matriz_interbancaria" in claves:
            ...
            print(
                " [WARN] Formato: INGLÉS (L_bb detectada). Los scripts nuevos podrían fallar."
            )
            matriz = datos["L_bb"]
        else:
            print(
                " [ERROR] No se encuentra matriz de préstamos (ni 'L_bb' ni 'matriz_interbancaria')."
            )
            return
        ...
        if total_prestamos == 0:
            print(
                " [!] ALERTA CRÍTICA: La simulación corrió pero NO hubo préstamos (Volumen 0)."
            )
            print(
                "     Causa probable: Tasas de interés muy altas, exceso de liquidez inicial o lógica de matching rota."
            )
        else:
